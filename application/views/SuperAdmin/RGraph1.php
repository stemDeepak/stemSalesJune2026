    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>BD Request</h5></center><hr>
...
<div style="width: 80%; margin: auto;">
   <canvas id="combinedChartID1"></canvas>
    ...
    <?php 
        ...
        var combinedData1 = {
            labels: [<?php $bdrtype = $this->Menu_model->get_bdrtype(); $i=1; foreach($bdrtype as $bdrt){?> '<?=$bdrt->request_type?>', <?php } ?>],
            datasets: [
                {
                    label: 'BD Request',
                    ...
                    data: [
                        <?php 
                            $bdrtype = $this->Menu_model->get_bdrtype(); $i=1; foreach($bdrtype as $bdrt){$rtype = $bdrt->request_type;
                            $task = $this->Menu_model->get_bdrbytype($uid,$rtype);
                            foreach($task as $ts){
                        ?>
                        <?=$ts->cont?>,
                        <?php }} ?>],
                ...
                <?php $ttype = array('PIA Not Assign','PIA Assign But not Start','Started but not Complete','Completed but not BD Close');?>
                ...
                <?php $i=1; for($j=0;$j<=3;$j++){ ?>
                {
                    ...
                    data: [
                        <?php 
                            $bdrtype = $this->Menu_model->get_bdrtype(); foreach($bdrtype as $bdrt){$rtype = $bdrt->request_type;
                            $task = $this->Menu_model->get_bdrbytypeandstage($uid,$rtype,$j);
                            foreach($task as $ts){
                        ?>
                        <?=$ts->cont?>,
                        <?php }} ?>
