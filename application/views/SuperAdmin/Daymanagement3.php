                ...
                <div class="card">
                  <div class="card-header">
                    ...
                    <div class="card">
                                    ...
                                    <div class="table-responsive">
                                        <div class="table-responsive">
                                           ...
                                           if(!empty($getReportbyUser) && is_array($getReportbyUser)) {
                                                foreach ($getReportbyUser as $userkey => $periods):
                                                ?>
                                                    <center><h2>Date: <?php echo getUserNameById($userkey); ?></h2></center>
                            <?php foreach ($periods as $period => $records):
                                if($period == 'Mornings'){
                                    $start = new DateTime($records[0]['user_start_time']);
                                    $end = new DateTime($records[0]['task_start_time']);
                                    $diff = $start->diff($end);
                                ?>
                        <table class="table">
                            ...
                            <tr>
                              ...
                              <th>Autotask Time</th>
                              <th>Plan task request</th>
                            </tr>
                          </thead>
                          <tbody>
                            <tr>
                              <td><?=getworkfrom($records[0]['wffo']); ?></td>
                              <td><?= $records[0]['ustart']; ?></td>
                              <td><img class="uimage" height="100px" alt="image not found" src="<?=base_url().'/'.$records[0]['usimg'];?>"></td>
                              <td><?= $records[0]['task_start_time'];?><br><?php echo $diff->format('%d days, %h hours, %i minutes, %s seconds'); ?></td>
                              <td><?= $records[0]['autotask']; ?></td>
                              <td><?php
                             if(!empty($records[0]['created_at'])){ 
                              ?>
                                    <b>Requested At : </b><?= $records[0]['created_at'] ?><br>
                                    <b>Request Remarks : </b><?= $records[0]['request_remarks'] ?><br>
                                    <b>Approval Status : </b><?=$records[0]['approvel_status'] ?><br>
                                    <b>Planner Approved at : </b> <?= $records[0]['updated_at']?><br>
                                    <b>Planner Approved by : </b> <?= $records[0]['approver_Name'] ?><br>
                                <?php }else{
                        ...
                        <?php }
                        else if($period == 'Yesterday Evening'){ ?>
                         <table class="table">
                            <thead class="thead-dark">
                                <tr>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td><a href="<?= base_url() . '/' . $records[0]['ucimg']; ?>">
                                        <img class="uimage" height="100px" alt="image not found" src="<?= base_url() . '/' . $records[0]['ucimg']; ?>">
                                        </a></td>
                                    <td data-question="total time taken to plan planner" data-userid="<?= $data->user_id; ?>" data-period="Yesterday Evening" data-cdate="<?= $cdate; ?>">
                                                            ...
                                                            <?= $records[0]['total_timeTakeFor_planner'] ?>
                                                        </td>
                                    <td><?= $records[0]['reasonFor_Request']; ?></td>
                                </tr>
                            </tbody>
                        <?php
                            }
                            else if($period == 'Yesterday Task'){ ?>
                            <table class="table">
                                <thead class="thead-dark">
                                    <tr>
                                ...
                                <tbody>
                                    <tr><td data-question="Yesterday Total Plan Task is Good" data-userid="<?= $records[0]['user_id']; ?>" data-period="Yesterday Task" data-cdate="<?= $sdate; ?>">
                                                            <?php $dayData = $this->Management_model->CheckingYesterDayTaskStatus($records[0]['user_id']); ?>
                                                            <b>Own funnel tasks : </b> 
                                                            <a class="text-primary" target="_BLANK" href="<?= base_url() . 'Management/CheckingYesterDayTask/total/' . $records[0]['user_id']. '/' . $sdate ?>">
                                                                <?= $dayData[0]->plan ?></a>
                                                                <hr>
                                                            <?php 
                                                                // $typeID = 13;
                                        ...
                                        <?php  } ?></td>
                                <td><a class="text-primary" target="_BLANK" href="<?= base_url() . 'Management/CheckingYesterDayTask/done/' . $records[0]['user_id'] . '/' . $cdate ?>"><?= $dayData[0]->done ?></a>
                                </td>
                                <td><b>Own funnel completed tasks : </b> 
                                                            ...
                                                            <?php } ?></td>
                                <td>  <b>Own funnel Total Auto Task : </b>
                          ...
                          <?php  }
                            ?> <h4>Periods: <?php echo htmlspecialchars($period); 
                                    ?></h4><table class="table" id="example_<?= $i ?>">
                                                                    <thead>
                                                                        <tr>
                                                                            <th>Question</th>
                                                                            <th>Star Rating</th>
                                                                    ...
                                                                    <tbody>
                                                                        <?php foreach ($records as $record): 
                                                                            ...
                                                                            <tr>
                                                                                ...
                                                                                <td><?php echo htmlspecialchars($record['scomment']); ?></td>
                                                                            </tr>
                                                                        <?php endforeach; ?>
                                                                    </tbody>
                                                                </table>
