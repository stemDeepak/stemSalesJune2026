    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>Category Wise Funnel Graph</h5></center><hr>
    ...
    google.charts.setOnLoadCallback(drawChart11);
      function drawChart11() {
        var data = google.visualization.arrayToDataTable([
         ...
         ["No Category (<?=$cat[0]->nocat?>)", <?=$cat[0]->nocat?>,"0","<?=$uid?>"],
         ["Top Spender (<?=$cat[0]->topspender?>)", <?=$cat[0]->topspender?>,"upsell_client","<?=$uid?>"],
         ["Focus Funnel (<?=$cat[0]->focus_funnel?>)", <?=$cat[0]->focus_funnel?>,"focus_funnel","<?=$uid?>"],
         ["Upsell Client (<?=$cat[0]->upsell_client?>)", <?=$cat[0]->upsell_client?>,"upsell_client","<?=$uid?>"],
         ["Key Client (<?=$cat[0]->keycompany?>)", <?=$cat[0]->keycompany?>,"keycompany","<?=$uid?>"],
         ["Positive Key Client (<?=$cat[0]->pkclient?>)", <?=$cat[0]->pkclient?>,"pkclient","<?=$uid?>"],
         ["Priority Client (<?=$cat[0]->priorityc?>)", <?=$cat[0]->priorityc?>,"priorityc","<?=$uid?>"],
              ...
              <div class="container-fluid card p-5" id="data-container">
                ...
                <div id="tabular-view" style="display: none;">
                   ...
                   <?php $cat = $this->Menu_model->get_fannalcat($uid);?>
                           ...
                           No Category (<?=$cat[0]->nocat?>)<br>
                           Top Spender (<?=$cat[0]->topspender?>)<br>
                           Focus Funnel (<?=$cat[0]->focus_funnel?>)<br>
                           Upsell Client (<?=$cat[0]->upsell_client?><br>
                           Key Client (<?=$cat[0]->keycompany?>)<br>
                           Positive Key Client (<?=$cat[0]->pkclient?>)<br>
                           Priority Client (<?=$cat[0]->priorityc?>)<br>
