...
<form class="p-3" method="POST" action="TaskGraph7">
  ...
  <script type="text/javascript">
    ...
    function drawChart() {
      var data = google.visualization.arrayToDataTable([
        ...
        <?php $avgtask = $this->Menu_model->get_avgtaskparday($uid,$sd,$ed); $i=0;foreach($avgtask as $avgt){?>
        ["<?=$avgt->action_name?>", <?=$avgt->avg_daily_tasks?>, "<?=$colors[$i]?>"],
        <?php $i++;} ?>
      ]);
    ...
    <script>
        var stackedData = {
            ...
            datasets: [
                ...
                <?php $i=0; $action = $this->Menu_model->get_action(); foreach($action as $ac){$acid = $ac->id; ?>
                {
                    label: '<?=$ac->name?>',
                    ...
                    data: [
                        ...
                        $stid = $st->id;
                          $task = $this->Menu_model->get_avgtasksamest($uid,$sd,$ed,$stid,$acid); foreach($task as $ta){?>
                        <?=$ta->cont?>,          
                        <?php }} ?>   
                        ],
                    stack: 'Tasks'
        ...
        var stackedData = {
            ...
            datasets: [
                        ...
                        $acid = $ac->id; ?>
                {
                    ...
                    data: [
                        <?php $timeslot = $this->Menu_model->get_timeslot(); foreach($timeslot as $tl){$t1=$tl->time1;$t2=$tl->time2;
                          $task = $this->Menu_model->get_timewtpbteac($uid,$sd,$ed,$t1,$t2,$acid); foreach($task as $ta){?>
                        <?=$ta->cont?>,          
                        <?php }} ?>   
                        ],
                    stack: 'Tasks'
                ...
                ?>
                {
                    ...
                    data: [
                        <?php $timeslot = $this->Menu_model->get_timeslot(); foreach($timeslot as $tl){$t1=$tl->time1;$t2=$tl->time2;
                          $task = $this->Menu_model->get_timewtpbteacst($uid,$sd,$ed,$t1,$t2,$stid,$acid); foreach($task as $ta){?>
                        <?=$ta->cont?>,          
                        <?php }} ?>          
        ...
        var stackedChart = new Chart(stackedCtx, {
            ...
            options: {
                plugins: {
                    title: {
                        display: true,
                        text: 'Time Wise Task Plan but not executed ',
                    },
                },
                scales: {
                    x: {
