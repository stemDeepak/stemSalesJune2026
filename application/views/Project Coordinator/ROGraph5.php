    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>My Region Status Wise Other and My School</h5></center><hr>
...
<div style="width: 80%; margin: auto;">
   <canvas id="combinedChartID1"></canvas>
    ...
    <?php 
        ...
        var combinedData1 = {
            labels: [<?php $mrstatus = $this->Menu_model->get_mrspdstatus($uid); $i=1; foreach($mrstatus as $mrs){?> '<?=$mrs->sname?>', <?php } ?>],
            datasets: [
                {
                    label: 'Other School',
                    ...
                    data: [
                        <?php 
                            $mrstatus = $this->Menu_model->get_mrspdstatus($uid); $i=1; foreach($mrstatus as $mrs){$stid = $mrs->sid;
                            $task = $this->Menu_model->get_myspdbystid($uid,$stid);
                            foreach($task as $ts){
                        ?>
                        <?=$ts->oscont?>,
                        <?php }} ?>],
