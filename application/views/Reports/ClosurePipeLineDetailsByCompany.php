                  ...
                  <div class="frame-layer-2">
                   <div class="frame-layer-3" style="background: linear-gradient(to right, #ffffff1c, #ffffff1c); border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); padding: 20px;">
                      <h1 class="m-0 premium-color-1" style="color: #1df719ff;">
                        🔍 Closure Pipeline By Details
                      </h1>
                      <hr style="background-color:#285d00;min-width:300px;">
                      <h4 class="premium-color-2 text-capitalize" style="color: #ffffffff;">
                        <a href="<?=base_url()?>Menu/CompanyDetails/<?= $cmpDatas[0]->cid; ?>" target="_blank"><?= $cmpDatas[0]->compname.' ( '. $cmpDatas[0]->cid .' )'; ?> </a>
                      </h4>
...
<hr style="background-color:#285d00">
              ...
              <div class="card-new-table">
                <div class="header">
              <div class="title">
                <h1>Sales / <span style="color: #1df719ff;"><?= ucwords(str_replace("_", " ", $ftype)); ?> / <?= $cmpDatas[0]->compname.' ( '. $cmpDatas[0]->cid .' )'; ?></span></h1>
    ...
    <thead style="color:#fff;background-color: #000;">
       <tr>
            ...
            <th>🔎 Task Complete By</th>
            <th>🔎 Appointment Date</th>
            
            <?php if($ftype == 'total_rp_meeting'){ ?>
            <th>🔎 Task Name</th>
            
            <th>🔎 Meeting Type</th>
            <th class="text-left">🔎 Short Mom</th>
            <?php } ?>

            <?php if($ftype == 'total_proposal_sent'){ ?> 
                <th>💰 Proposal Budget</th>
            <?php } ?>
            <?php if(in_array($ftype,['total_proposal_sent','total_rp_meeting'])){ ?>
                <th> View Details</th>
            <?php } ?>
            <?php if($ftype == 'total_call_connected_after_rp_meeting' 
            || $ftype == 'total_line_manager_call_connected_after_rp_meeting'
            || $ftype == 'total_positive_conversions_call_after_rp_meeting'
            || $ftype == 'total_negative_conversions_call_after_rp_meeting'
            || $ftype == 'total_AY_PY_call_connected_after_rp_meeting'
            ){ ?>
                <th> Task Name</th>
                <th>🔎 On Status</th>
                <th>🔎 New Status</th>
                <th class="text-left"> Remarks</th>
                <th class="text-left"> Special Remarks</th>
            <?php } ?>



            <?php if($ftype == 'total_call_connected_after_proposal_sent' 
            || $ftype == 'total_line_manager_call_connected_after_proposal_sent' 
            || $ftype == 'total_positive_conversions_call_after_proposal_sent'
            || $ftype == 'total_negative_conversions_call_after_proposal_sent'
            || $ftype == 'total_AY_PY_call_connected_after_proposal_sent'
            ){ ?>
                 <th> Task Name</th>
                <th>🔎 On Status</th>
                <th>🔎 New Status</th>
    ...
    <tbody>
        ...
        <tr title='task ID - <?= $row->task_id ?>'>
                ...
                <td><?= $row->current_status_name ?></td>

                <?php if($ftype == 'total_rp_meeting'){ ?>
                    <td> <span><?= $row->meeting_done_by ?></span></td>
                <?php } ?> 
                <?php if($ftype == 'total_proposal_sent'){ ?>
                    <td> <span><?= $row->proposal_by ?></span></td>
                <?php } ?> 
                <?php if($ftype == 'total_call_connected_after_rp_meeting' 
                || $ftype == 'total_line_manager_call_connected_after_rp_meeting'
                || $ftype == 'total_positive_conversions_call_after_rp_meeting'
                ...
                || $ftype == 'total_AY_PY_call_connected_after_rp_meeting'
                    ...
                    <td> <span><?= $row->call_by_after_rp ?></span></td>
                    <td> <span><?= $row->call_by_after_rp_date ?></span></td>
                    <td class="text-left"> <span><?= $row->call_by_after_rp_call ?></span></td>
                     <td> <span><?= $row->call_by_after_rp_on_status ?></span></td>
                    <td> <span><?= $row->call_by_after_rp_new_status ?></span></td>
                    <td class="text-left"> <span><?= $row->call_by_after_rp_remarks ?></span></td>
                     <td><?php
                        if (
                            !empty($row->call_by_after_rp_special_remarks) &&
                            $row->call_by_after_rp_special_remarks !== null &&
                            $row->call_by_after_rp_special_remarks !== 'null' &&
                            $row->call_by_after_rp_special_remarks !== '[]'
                        ) { ?>
                           <button type="button" class="btn btn-primary"  onclick="SpecialRemarks(<?=$row->call_by_after_rp_task_id ?>)">view</button>
                       <?php }else{
                          echo '';
                        }
                    ?></td>
                ...
                || $ftype == 'total_AY_PY_call_connected_after_proposal_sent'
                  ...
                  <td> <span><?= $row->call_by_after_rp_proposal ?></span></td>
                    <td> <span><?= $row->call_by_after_rp_pdate ?></span></td>
                    
                    <td> <span>Call</span></td>

                    <td> <span><?= $row->call_by_after_p_on_status ?></span></td>
                    <td> <span><?= $row->call_by_after_p_new_status ?></span></td>

                    <td class="text-left"> <span><?= $row->call_by_after_rp_premarks ?></span></td>

                    <td><?php
                        if (
                            !empty($row->call_by_after_rp_pspecial_remarks) &&
                            $row->call_by_after_rp_pspecial_remarks !== null &&
                            $row->call_by_after_rp_pspecial_remarks !== 'null' &&
                            $row->call_by_after_rp_pspecial_remarks !== '[]'
                        ) { ?>
                           <button type="button" class="btn btn-primary"  onclick="SpecialRemarks(<?=$row->call_by_after_rp_ptask_id ?>)">view</button>
                       <?php }else{
                          echo '';
                        }
                    ?></td>
                <?php } ?> 
                 ...
                 <?php if($ftype == 'total_proposal_sent'){ ?>
                    <td> <span><?= $row->proposal_date ?></span></td>
                <?php } ?>

                <?php if($ftype == 'total_rp_meeting'){ ?>
                <td> <span><?= $row->appointmentdatetime ?></span></td>
                <td> <span><?= $row->meeting_task_name ?></span></td>
                <td class="text-left"> <?= $row->mtype ?></td>
                <td class="text-left"> 
                ...
                ?>
                  ...
                  <?php if($ftype == 'total_proposal_sent'){ ?> 
                    <td> <span style="color: rgb(24 243 0);"><?= formatIndianUnits($row->proposal_budgetme) ?></span></td> 
                 <?php } ?> 
                 
                <?php if($ftype == 'total_rp_meeting'){ ?>   
                    <td><a href="<?=base_url()?>Reports/ViewMeetingDetails/<?= $row->task_id; ?>" target="_blank">view </a></td>
                <?php } ?> 

                <?php if($ftype == 'total_proposal_sent'){ ?> 
                    <td><a href="<?=base_url()?>Reports/ViewProposalDetails/<?= $row->proposal_sid; ?>" target="_blank">view </a></td>
                <?php } ?>    
            </tr>
        <?php $k++; } ?>
    </tbody>
