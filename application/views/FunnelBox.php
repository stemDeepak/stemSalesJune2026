...
function drawChart42() {
  ...
  <?php
    ...
    foreach($status as $st){ 
    $stid = $st->id;
    $swtc = $this->Menu_model->get_taskolsna($uid,$stid,'1');
    foreach($swtc as $sw){ $url="https://stemapp.in/Menu"; ?>
      data.addRow(['<?=$st->name?> (<?=$sw->cont?>)',
      <?=$sw->a?>, '<?php if($sw->a == 0){echo "";}else{ echo $sw->a;}?>',
      <?=$sw->b?>, '<?php if($sw->b == 0){echo "";}else{ echo $sw->b;}?>']);
...
function drawChart43() {
  ...
  <?php
    ...
    foreach($status as $st){ 
    $stid = $st->id;
    $swtc = $this->Menu_model->get_taskolsna($uid,$stid,'10');
    foreach($swtc as $sw){ $url="https://stemapp.in/Menu"; ?>
      data.addRow(['<?=$st->name?> (<?=$sw->cont?>)',
      <?=$sw->a?>, '<?php if($sw->a == 0){echo "";}else{ echo $sw->a;}?>',
      <?=$sw->b?>, '<?php if($sw->b == 0){echo "";}else{ echo $sw->b;}?>']);
...
function drawChart44() {
  ...
  <?php
    ...
    foreach($status as $st){ 
    $stid = $st->id;
    $swtc = $this->Menu_model->get_taskolsna($uid,$stid,'2');
    foreach($swtc as $sw){ $url="https://stemapp.in/Menu"; ?>
      data.addRow(['<?=$st->name?> (<?=$sw->cont?>)',
      <?=$sw->a?>, '<?php if($sw->a == 0){echo "";}else{ echo $sw->a;}?>',
      <?=$sw->b?>, '<?php if($sw->b == 0){echo "";}else{ echo $sw->b;}?>']);
...
function drawChart45() {
  ...
  <?php
    ...
    foreach($status as $st){ 
    $stid = $st->id;
    $swtc = $this->Menu_model->get_taskolsna($uid,$stid,'3');
    foreach($swtc as $sw){ $url="https://stemapp.in/Menu"; ?>
      data.addRow(['<?=$st->name?> (<?=$sw->cont?>)',
      <?=$sw->a?>, '<?php if($sw->a == 0){echo "";}else{ echo $sw->a;}?>',
      <?=$sw->b?>, '<?php if($sw->b == 0){echo "";}else{ echo $sw->b;}?>']);
...
function drawChart46() {
  ...
  <?php
    ...
    foreach($status as $st){ 
    $stid = $st->id;
    $swtc = $this->Menu_model->get_taskolsna($uid,$stid,'4');
    foreach($swtc as $sw){ $url="https://stemapp.in/Menu"; ?>
      data.addRow(['<?=$st->name?> (<?=$sw->cont?>)',
      <?=$sw->a?>, '<?php if($sw->a == 0){echo "";}else{ echo $sw->a;}?>',
      <?=$sw->b?>, '<?php if($sw->b == 0){echo "";}else{ echo $sw->b;}?>']);
...
function drawChart47() {
  ...
  <?php
    ...
    foreach($status as $st){ 
    $stid = $st->id;
    $swtc = $this->Menu_model->get_taskolsna($uid,$stid,'5');
    foreach($swtc as $sw){ $url="https://stemapp.in/Menu"; ?>
      data.addRow(['<?=$st->name?> (<?=$sw->cont?>)',
      <?=$sw->a?>, '<?php if($sw->a == 0){echo "";}else{ echo $sw->a;}?>',
      <?=$sw->b?>, '<?php if($sw->b == 0){echo "";}else{ echo $sw->b;}?>']);
...
function drawChart48() {
  ...
  <?php
    ...
    foreach($status as $st){ 
    $stid = $st->id;
    $swtc = $this->Menu_model->get_taskolsna($uid,$stid,'6');
    foreach($swtc as $sw){ $url="https://stemapp.in/Menu"; ?>
      data.addRow(['<?=$st->name?> (<?=$sw->cont?>)',
      <?=$sw->a?>, '<?php if($sw->a == 0){echo "";}else{ echo $sw->a;}?>',
      <?=$sw->b?>, '<?php if($sw->b == 0){echo "";}else{ echo $sw->b;}?>']);
...
function drawChart49() {
  ...
  <?php
    ...
    foreach($status as $st){ 
    $stid = $st->id;
    $swtc = $this->Menu_model->get_taskolsna($uid,$stid,'7');
    foreach($swtc as $sw){ $url="https://stemapp.in/Menu"; ?>
      data.addRow(['<?=$st->name?> (<?=$sw->cont?>)',
      <?=$sw->a?>, '<?php if($sw->a == 0){echo "";}else{ echo $sw->a;}?>',
      <?=$sw->b?>, '<?php if($sw->b == 0){echo "";}else{ echo $sw->b;}?>']);
...
function drawChart26() {
  var data = new google.visualization.DataTable();
  data.addColumn('string', 'Action Y Purpose N');
  data.addColumn('number', 'Call');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Email');
  data.addColumn({type: 'string', role: 'annotation'});
  ...
  foreach($status as $st){ 
    $stid = $st->id;
    $swtc = $this->Menu_model->get_statuswisetaskandaypy($uid,$stid,$sd,$ed);
    foreach($swtc as $sw){
        $url="https://stemapp.in/Menu";
      ?>
      data.addRow(['<?=$st->name?> ( <?=$sw->cont?>)',
...
function drawChart27() {
  ...
  foreach($status as $st){ 
    $stid = $st->id;
    $swtc = $this->Menu_model->get_statuswisetaskandaypn($uid,$stid,$sd,$ed);
    foreach($swtc as $sw){
        $url="https://stemapp.in/Menu";
      ?>
      data.addRow(['<?=$st->name?> ( <?=$sw->cont?>)',
...
function drawChart29() {
  ...
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Reachout');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Tentative');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Will do Later');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Not Interested');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Positive');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Closure');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'OPEN RPEM');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Very Positive');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'TTD-Reachout');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'WNO-Reachout');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Positive-NAP');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Very Positive-NAP');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'On-Boarded');
  data.addColumn({type: 'string', role: 'annotation'});

  <?php
  
    $fruits = array("topspender", "upsell_client", "focus_funnel","keycompany","pkclient","priorityc");
    for ($i = 0; $i < count($fruits); $i++) {
        $cat = $fruits[$i];
    
    $swtc = $this->Menu_model->get_catwisestatus($uid,$cat);
    foreach($swtc as $sw){
        $url="https://stemapp.in/Menu";
      ?>
      data.addRow(['<?=$cat?> ( <?=$sw->cont?>)',
      <?=$sw->a?>, '<?php if($sw->a == 0){echo "";}else{ echo $sw->a;}?>',
      <?=$sw->b?>, '<?php if($sw->b == 0){echo "";}else{ echo $sw->b;}?>',
      <?=$sw->c?>, '<?php if($sw->c == 0){echo "";}else{ echo $sw->c;}?>',
      <?=$sw->d?>, '<?php if($sw->d == 0){echo "";}else{ echo $sw->d;}?>',
      ...
      <?=$sw->i?>, '<?php if($sw->i == 0){echo "";}else{ echo $sw->i;}?>',
      <?=$sw->j?>, '<?php if($sw->j == 0){echo "";}else{ echo $sw->j;}?>',
      <?=$sw->k?>, '<?php if($sw->k == 0){echo "";}else{ echo $sw->k;}?>',
      <?=$sw->k?>, '<?php if($sw->l == 0){echo "";}else{ echo $sw->l;}?>',
      <?=$sw->k?>, '<?php if($sw->m == 0){echo "";}else{ echo $sw->m;}?>',
      <?=$sw->k?>, '<?php if($sw->n == 0){echo "";}else{ echo $sw->n;}?>']);
      <?php
    }}
  ?>
  ...
  google.visualization.events.addListener(chart, 'select', function() {
    ...
    if (selection.length > 0) {
      var rowIndex = selection[0].row;
      var hyperlink = data.getValue(rowIndex, 26);
      if (hyperlink) {
        window.location.href = hyperlink;
      }
    }
...
function drawChart30() {
  ...
  <?php
    ...
    $ounmfunnal = $this->Menu_model->get_ounmfunnal($uid,$sd,$ed);
    foreach($ounmfunnal as $ounmf){
        $ouname = $ounmf->ouname;
        $ouid = $ounmf->ouid;
    
    $swtc = $this->Menu_model->get_outaskonmyfunnalaypy($uid,$ouid,$sd,$ed);
    foreach($swtc as $sw){
        $url="https://stemapp.in/Menu";
      ?>
      data.addRow(['<?=$ouname?> ( <?=$sw->cont?>)',
      <?=$sw->a?>, '<?php if($sw->a == 0){echo "";}else{ echo $sw->a;}?>',
      <?=$sw->b?>, '<?php if($sw->b == 0){echo "";}else{ echo $sw->b;}?>',
      <?=$sw->c?>, '<?php if($sw->c == 0){echo "";}else{ echo $sw->c;}?>',
      <?=$sw->d?>, '<?php if($sw->d == 0){echo "";}else{ echo $sw->d;}?>',
...
function drawChart31() {
  ...
  <?php
    ...
    $swtc = $this->Menu_model->get_outaskonmyfunnalaypn($uid,$ouid,$sd,$ed);
    foreach($swtc as $sw){
        $url="https://stemapp.in/Menu";
      ?>
      data.addRow(['<?=$ouname?> ( <?=$sw->cont?>)',
...
function drawChart32() {
  ...
  <?php
    ...
    $swtc = $this->Menu_model->get_ouactionnmyfunnalaypy($uid,$ouid,$sd,$ed);
    foreach($swtc as $sw){
        $url="https://stemapp.in/Menu";
      ?>
      data.addRow(['<?=$ouname?> ( <?=$sw->cont?>)',
...
function drawChart33() {
  ...
  <?php
    ...
    $swtc = $this->Menu_model->get_ouactionnmyfunnalaypn($uid,$ouid,$sd,$ed);
    foreach($swtc as $sw){
        $url="https://stemapp.in/Menu";
      ?>
      data.addRow(['<?=$ouname?> ( <?=$sw->cont?>)',
...
function drawChart34() {
  ...
  data.addColumn('number', 'Open-RPEM');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'RPEM-Reachout');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Open-Reachout');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Reachout-Tentative');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Tentative-Positive');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'Positive-VPositive');
  data.addColumn({type: 'string', role: 'annotation'});
  data.addColumn('number', 'VPositive-Clouser');
  data.addColumn({type: 'string', role: 'annotation'});
  <?php
  
    $ounmfunnal = $this->Menu_model->get_ouconmyfunnal($uid,$sd,$ed);
    foreach($ounmfunnal as $ounmf){
        $ouname = $ounmf->ouname;
        $ouid = $ounmf->ouid;
    
    $swtc = $this->Menu_model->get_ouconmyfunnaldata($uid,$ouid,$sd,$ed);
    foreach($swtc as $sw){
        $url="https://stemapp.in/Menu";
      ?>
      data.addRow(['<?=$ouname?>',
  ...
  google.visualization.events.addListener(chart, 'select', function() {
    ...
    if (selection.length > 0) {
      var rowIndex = selection[0].row;
      var hyperlink = data.getValue(rowIndex, 6);
      if (hyperlink) {
        window.location.href = hyperlink;
      }
    }
...
function drawChart35() {
  ...
  <?php
    ...
    $swtc = $this->Menu_model->get_ouconmyfunnaldataaypn($uid,$ouid,$sd,$ed);
    foreach($swtc as $sw){
        $url="https://stemapp.in/Menu";
      ?>
      data.addRow(['<?=$ouname?>',
...
<script type="text/javascript">
  ...
  google.charts.setOnLoadCallback(drawChart51);
    function drawChart51() {
      var data = google.visualization.arrayToDataTable([
        ["Conversion", "Task", { role: "style" } ],
        <?php 
            $stage1 = $this->Menu_model->get_avgtaskofscon($uid,$sd,$ed,'1','8');
            $stage2 = $this->Menu_model->get_avgtaskofscon($uid,$sd,$ed,'8','2');
            $stage3 = $this->Menu_model->get_avgtaskofscon($uid,$sd,$ed,'1','2');
            $stage4 = $this->Menu_model->get_avgtaskofscon($uid,$sd,$ed,'2','3');
            $stage5 = $this->Menu_model->get_avgtaskofscon($uid,$sd,$ed,'3','6');
            $stage6 = $this->Menu_model->get_avgtaskofscon($uid,$sd,$ed,'6','9');
            $stage7 = $this->Menu_model->get_avgtaskofscon($uid,$sd,$ed,'9','7');
        ?>
        
        ["Open-RPEM", <?=$stage1[0]->ttask?>, "color: #76A7FA"],
        ["RPEM-Reachout", <?=$stage2[0]->ttask?>, "color: #76A7FA"],
        ["Open-Reachout", <?=$stage3[0]->ttask?>, "color: #76A7FA"],
        ["Reachout-Tentative", <?=$stage4[0]->ttask?>, "color: #76A7FA"],
        ["Tentative-Positive", <?=$stage5[0]->ttask?>, "color: #76A7FA"],
        ["Positive-VPositive", <?=$stage6[0]->ttask?>, "color: #76A7FA"],
        ["VPositive-Clouser", <?=$stage7[0]->ttask?>, "color: #76A7FA"],
      ]);
      ...
      var options = {
        title: "Average Task Sales Cycle",
        height: 500,
        bar: {groupWidth: "95%"},
        legend: { position: "none" },
      };
...
<form class="p-3" method="POST" action="FunnelBox">
