...
// echo "<pre>"; print_r($mdata);exit;
                     foreach($mdata as $md){
                      ...
                      ?>
                     <div class="alert alert-light" role="alert" style="background-color: #bbddff;" id="<?=$md->aprid?>">
                        <b><?= '[ '.$i .' ]  '.$md->compname?> | <?=$md->name?> |  <?=$md->currentstatus?>| 
                        <a class="bg-primary p-2 m-1 hoverbox" href="<?=base_url();?><?=$md->proattach?>" target='_blank' class="text-primary">View</a> | 
                        <span class="bg-success p-2 m-1 hoverbox" onclick="Apr('<?=$md->aprid?>');" id="<?=$md->aprid?>">Approve</span> | 
                        <span class="bg-danger p-2 m-1 hoverbox" onclick="Rej('<?=$md->aprid?>');" id="<?=$md->aprid?>">Reject</span>
                        </b>
                     </div>
                     <?php $i++; }
                     }
