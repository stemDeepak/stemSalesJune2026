...
<?php include("nav.php"); ?>
    ...
    <div class="card">
        ...
        <h3>User has marked the current year's focus funnels and added them to the planner.</h3>
        </div>
    </div>
    <hr>
        <form class="filters">
    ...
    <script>
        ...
        function applyFilters() {
            ...
            const filteredData = data.filter(item =>
                ...
                (startDate === '' || new Date(item.task_date) >= new Date(startDate)) &&
                (endDate === '' || new Date(item.task_date) <= new Date(endDate))
            );
