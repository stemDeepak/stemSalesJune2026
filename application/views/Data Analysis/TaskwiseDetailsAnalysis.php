...
$(document).ready(function () {
    ...
    function filterData() {
        return rawData.filter(d => {
            return <?php foreach ($filters as $filter) { if ($filter !== 'task_date') { echo "($('#$filter' + 'Filter').val() === 'all' || d.$filter === $('#$filter' + 'Filter').val() || ($('#$filter' + 'Filter').val() === '(Blank)' && d.$filter === '')) && "; } } ?> true;
        });
    }
