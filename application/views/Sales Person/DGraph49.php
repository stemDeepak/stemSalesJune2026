    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>Time Wise Auto Task Plan/Inisated/Updated</h5></center><hr>
...
<div style="width: 80%; margin: auto;">
   <canvas id="combinedChartID1"></canvas>
    ...
    $ed=date('Y-m-d');?>
        ...
        var combinedData1 = {
            labels: [<?php $times = $this->Menu_model->get_timeslot(); $i=1; foreach($times as $ts){ $t1 = $ts->time1;$t2 = $ts->time2;?> '<?=$ts->time1?> to <?=$ts->time2?>', <?php } ?>],
            datasets: [
                {
                    label: 'Auto Task Updated',
                    ...
                    data: [
                        ...
                        $task = $this->Menu_model->get_autotasktypeupdatebytp($uid,$sd,$ed,$t1,$t2);
                        foreach($task as $ts){
                        ?>
                        <?=$ts->cont?>,
                        <?php }} ?>],
                ...
                
                {
                    ...
                    data: [
                        <?php 
                        $times = $this->Menu_model->get_timeslot(); $i=1; foreach($times as $ts){ $t1 = $ts->time1;$t2 = $ts->time2;
                        $task = $this->Menu_model->get_autotasktypeupdatebyti($uid,$sd,$ed,$t1,$t2);
                        foreach($task as $ts){
                        ?>
                        <?=$ts->cont?>,
                        <?php }} ?>],
                ...
                
                {
                    ...
                    data: [
                        <?php 
                        $times = $this->Menu_model->get_timeslot(); $i=1; foreach($times as $ts){ $t1 = $ts->time1;$t2 = $ts->time2;
                        $task = $this->Menu_model->get_autotasktypeupdatebytu($uid,$sd,$ed,$t1,$t2);
                        foreach($task as $ts){
                        ?>
                        <?=$ts->cont?>,
                        <?php }} ?>],
