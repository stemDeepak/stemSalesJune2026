...
/* Hide non-active items to the left of the active one */
    ...
    <section class="content">
      <div class="container-fluid card p-5 ">
          <div class="row text-center">
                <?php 
                foreach($mdata as $dt){ $tid = $dt->tid;
                 $sid = $dt->sid;
                 $report=$this->Menu_model->get_reportbystid($tid,$sid);
                 $comm=$this->Menu_model->get_commbystid($tid,$sid);
                 $wgdata=$this->Menu_model->get_wgbytttid($tid);
                 
                 if($dt->action=='Visit'){$url=base_url()."Menu/SPDVISITTASKBOX/".$sid.'/'.$tid;}
                 else{$url="#";}
                 
                 
                 $cdate=date('Y-m-d H:i:s');?>
                    <div class="col-sm-12 col-md-4 col-lg-4 mb-4">
                      <a href="<?=base_url();?>Menu/SPDPANTASK/<?=$tid?>" target="_blank">
                      <div class="card p-3 border rounded border-success hover-div d-flex flex-column align-items-stretch h-100">
                            Client Name<br><b><?=$dt->client_name?></b><hr>
                             ...
                             Status<br><b><?=$dt->stname?></b><hr>
                             Task By<br><b><?=$dt->uname?></b><hr>
                             Task Date<br><b><?=$dt->taskudt?></b><hr>
                             Action<br><b><?=$dt->action?></b><hr>
                             Remark<br><b><?=$dt->tremark?></b><hr></a>
                             <?php if($comm){ foreach($comm as $com) { ?>
                             <img src="https://stemoppapp.in/<?=$com->filepath?>" class="img-thumbnail">
                             <?php }} ?>
...
<script>
    ...
    // Set an interval to automatically advance to the next item
    const interval = setInterval(nextCarouselItem, 3000); // Change the interval duration as desired (in milliseconds)

    // Stop the automatic carousel rotation on hover
    document.querySelector(".carousel").addEventListener("mouseenter", function() {
        clearInterval(interval);
    });

    // Resume the automatic carousel rotation on mouse leave
    document.querySelector(".carousel").addEventListener("mouseleave", function() {
        interval = setInterval(nextCarouselItem, 3000);
    });
});
