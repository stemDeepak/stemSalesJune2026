    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>Updated Task AYPY/AYPN/ANPN</h5></center><hr>
   ...
   <canvas id="combinedChartID1"></canvas>
    ...
    $ed=date('Y-m-d');?>
        ...
        var combinedData1 = {
            ...
            datasets: [
                {
                    label: 'Acton Taken Yes and Purpose Achieved Yes',
                    ...
                    data: [
                        ...
                        $task = $this->Menu_model->get_tasktypeupdatebyauaypy($uid,$sd,$ed,$acid);
                        foreach($task as $ts){
                        ?>
                        <?=$ts->cont?>,
                        <?php }} ?>],
                ...
                {
                    label: 'Acton Taken Yes and Purpose Achieved No',
                    backgroundColor: ['blue','blue','blue','blue','blue','blue','blue','blue','blue','blue','blue','blue','blue','blue'],
                    data: [
                        <?php 
                        $action = $this->Menu_model->get_action(); $i=1; foreach($action as $ac){ $acid = $ac->id;
                        $task = $this->Menu_model->get_tasktypeupdatebyauaypn($uid,$sd,$ed,$acid);
                        foreach($task as $ts){
                        ?>
                        <?=$ts->cont?>,
                        <?php }} ?>],
                ...
                {
                    label: 'Acton Taken No and Purpose Achieved No',
                    backgroundColor: ['green','green','green','green','green','green','green','green','green','green','green','green','green','green'],
                    data: [
                        <?php 
                        $action = $this->Menu_model->get_action(); $i=1; foreach($action as $ac){ $acid = $ac->id;
                        $task = $this->Menu_model->get_tasktypeupdatebyauanpn($uid,$sd,$ed,$acid);
                        foreach($task as $ts){
                        ?>
                        <?=$ts->cont?>,
                        <?php }} ?>],
