    ...
    <section class="content">
      <div class="container-fluid">
          <div class="card col-12  p-3 text-center">
              <center><h5>My Region Year Wise Other and My Project</h5></center><hr>
...
<div style="width: 80%; margin: auto;">
   <canvas id="combinedChartID1"></canvas>
    ...
    <?php 
        ...
        var combinedData1 = {
            labels: [<?php $mryear = $this->Menu_model->get_mrspdyear($uid); $i=1; foreach($mryear as $mry){?> '<?=$mry->pyear?>', <?php } ?>],
            datasets: [
                {
                    label: 'Other Project',
                    ...
                    data: [
                        <?php 
                            $mryear = $this->Menu_model->get_mrspdyear($uid); $i=1; foreach($mryear as $mry){$pyear = $mry->pyear;
                            $task = $this->Menu_model->get_myspdbyyear($uid,$pyear);
                            foreach($task as $ts){
                        ?>
                        <?=$ts->oscont?>,
                        <?php }} ?>],
                ...
                
                {
                    ...
                    data: [
                        <?php 
                            $mryear = $this->Menu_model->get_mrspdyear($uid); $i=1; foreach($mryear as $mry){$pyear = $mry->pyear;
                            $task = $this->Menu_model->get_myprojectbyyear($uid,$pyear);
                            foreach($task as $ts){
                        ?>
                        <?=$ts->mscont?>,
                        <?php }} ?>],
