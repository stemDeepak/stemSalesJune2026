...
foreach($cmeetingsDatas as $meeting) {
    ...
    echo '<td style="width:50%; vertical-align:top;">
            ...
            <p><strong>Initial Photo:</strong> ' . (!empty($meeting->initphoto) ? '<a target="_blank" href="'.base_url().'uploads/day/'.$meeting->initphoto.'">View</a>' : '-') . '</p>
            <p><strong>Current Photo:</strong> ' . (!empty($meeting->cphoto) ? '<a target="_blank" href="'.base_url().$meeting->cphoto.'">View</a>' : '-') . '</p>
          </td>';
    echo '</tr>';
}
