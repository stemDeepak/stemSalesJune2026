              ...
              <!-- <div>
                  <form class="p-3" method="POST" action="">
                   ...
                   <div class="card-header text-center" style="background: linear-gradient(to right, #fff8e1, #ffffff); border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); padding: 20px;">
                    <h3 >📊 Minutes of Meeting (MoM Status)</h3>
                    <small>The MoM Status 📑 provides a clear overview of all meeting records, including total written MoM ✍️, pending review ⏳, approved ✅, and rejected ❌ data. It helps track progress, ensures accountability 🤝, and highlights the current stage of every meeting discussion 🗂️. With MoM Status, teams can stay aligned, identify gaps 🔍, and maintain transparency for effective decision-making 🚀.</small>
                  </div>  

                  <?php 
                  
                  $mdata = $this->Menu_model->GetAllUserByRollesLogins($uid);
                                ...
                                <table id="example1" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                  <thead class="thead-dark">
                                    <tr>
                                      ...
                                      <th>⏳ Pending MOM</th>
                                      <th>✅ Approved MOM</th>
                                      <th>❌ Rejected For Reupdate MOM</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    <?php 
                                    ...
                                    <tr>
                                      ...
                                      <td><?=$dt->name?></td>
                                      <td>
                                        <a target="_blank" class="btn btn-primary rounded" 
                                          href="<?= base_url().'Management/TotalMomData/'.$dt->user_id.'/'.$cdate; ?>">
                                          <?= $mom_Data_total_cnt ?>
                                        </a>
                                      </td>
                                      <td>
                                        <a target="_blank" class="btn btn-warning rounded" 
                                          href="<?= base_url().'Management/PendingTeamMomData/'.$dt->user_id.'/'.$cdate; ?>">
                                          <?= $mom_Datacnt ?>
                                        </a>
                                      </td>
                                      <td>
                                        <a target="_blank" class="btn btn-success rounded" 
                                          href="<?= base_url().'Management/TotalApproveMomData/'.$dt->user_id.'/'.$cdate; ?>">
                                          <?= $mom_Data_appr_cnt ?>
                                        </a>
                                      </td>
                                      <td>
                                        <a target="_blank" class="btn btn-danger rounded" 
                                          href="<?= base_url().'Management/MomDataReject/'.$dt->user_id.'/'.$cdate; ?>">
                                          <?= $mom_Data_Reject_cnt ?>
                                        </a>
                                      </td>
