     ...
     <?php $this->load->view($dep_name.'/nav.php');?>
      <div class="content-wrapper">
        ...
        <section class="content">
          <div class="container-fluid" style="background: linear-gradient(to right, #e1f8ffff, #ffffff); border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
            <div class="row">
              <div class="col-12">
                     <style>
...
tbody tr td:nth-child(2) {
    ...
    color: black;
                 ...
                 <div class="card mt-2" style="background: linear-gradient(to right, #e8f5e9, #ffffff); border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); padding: 20px;">
                  <div class="card-header text-center" style="background: linear-gradient(to right, #fff8e1, #ffffff); border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.1); padding: 20px;">
                    <h3 >📝 Minutes of Meeting (Rejected MoM Data)</h3>
                    <small>The Rejected MoM Data ❌ highlights meeting records that were not approved due to incomplete details, errors, or deviations from agreed points ⚠️. It helps identify gaps in documentation and ensures accuracy before final approval 📑. By reviewing rejected MoM, teams can refine processes 🔄, correct mistakes, and maintain transparency 🤝. This step is vital to ensure that only verified and reliable information is stored for future reference 📌.</small>
                  </div>
  ...
  <tbody>
    <?php 
    $i=1;
    $momdata = $this->Management_model->getAllRejectBDMoMData($suid,$tardate);
    foreach($momdata as $dt){
      ...
      $action  = $this->Menu_model->get_actionbyid($dt->action_id);
    ...
    <tr>
      ...
      <td><?= $remarks ?></td>
      <td>
        <a target="_blank" class="btn btn-primary" href="<?= base_url('Management/MomDataDisplay/'.$dt->user_id.'/'.$tardate.'/'.$dt->id) ?>">
          View Mom Details
        </a>
      </td>
      <td>
        <a target="_blank" class="btn btn-primary" href="<?= base_url('Reports/ViewMeetingDetails/'.$aftertask) ?>">
      ...
      <td>
        <?php if($dt->approved_by !== ''): ?>
          ...
          <?php elseif($approved_status == 'Reject'): ?>
            <span class="bg-danger p-1">Rejected</span>
          <?php else: ?>
            <span class="bg-warning p-1">Pending</span>
          <?php endif; ?> |
          <b><?= $approved_status ?> By:</b> <?= $approved_byname ?> | 
          <b>Date:</b> <?= $approved_date ?> | 
      ...
      <td>
        <?php 
        if($user['user_id'] == $suid){ 
          if($approved_status == 'Reject' && ($edit_cnt == '' || $edit_cnt == 0)){ ?>
            <a target="_blank" href="<?= base_url('Management/MomEditByUser/'.$dt->id.'/edit/'.$suid.'/'.$tardate) ?>" class="btn btn-primary">Edit</a>
          <?php 
          }
        } else { ?>
          <b>Status :</b> 
